...
test('normalizes SPAR market formats to the shared spar retailer key', () => {
  assert.deepEqual(normalizeRetailerList('SPAR,INTERSPAR,EUROSPAR'), ['spar']);
  assert.deepEqual(normalizeRetailerList(['INTERSPAR', 'eurospar']), ['spar']);
});

test('program retailer eligibility keeps selected retailer program offers visible when opted in', () => {
  ...
  assert.deepEqual(
    applyProgramEligibility([publicOffer, programOffer], { programRetailers: [] }).map((item) => item._id),
    ['spar-public']
  );
  assert.deepEqual(
    applyProgramEligibility([publicOffer, programOffer], { programRetailers: ['spar'] }).map((item) => item._id),
    ['spar-public', 'spar-program']
  );
});
...
test('dampens or excludes butter side hits when no real butter exists', () => {
  const offers = [
    offer({
      title: 'Peanut Butter Cups',
      categoryPrimary: 'Lebensmittel',
      categorySecondary: 'Suesswaren & Knabbereien',
      comparisonGroup: 'peanut-butter-cups::0.04-kg',
    }),
    offer({
    ...
    offer({
      title: 'Protein Cookie Almond Butter',
      categoryPrimary: 'Lebensmittel',
      categorySecondary: 'Suesswaren & Knabbereien',
      comparisonGroup: 'protein-cookie-almond-butter::0.05-kg',
    }),
  ];
...
test('does not make pasta reis konserven category-only offers strong rice hits', () => {
  const passata = offer({
    title: 'Tomaten Passata',
    categoryPrimary: 'Lebensmittel',
    categorySecondary: 'Pasta, Reis & Konserven',
    subcategoryKey: 'pasta-reis-konserven',
    comparisonGroup: 'tomaten-passata::0.7-kg',
  });

  assert.equal(scoreOfferAgainstQuery(passata, 'reis'), 0);
});

test('does not match random reis substrings as generic rice intent', () => {
  const offers = [
    offer({
      title: 'Darbo Preiselbeer Konfituere',
      categoryPrimary: 'Lebensmittel',
      categorySecondary: 'Obst & Gemuese',
      comparisonGroup: 'darbo-preiselbeer-konfituere::0.45-kg',
    }),
    offer({
      title: 'Framstagspreis Getraenke Aktion',
      categoryPrimary: 'Getraenke',
      categorySecondary: 'Bier',
      comparisonGroup: 'framstagspreis-getraenke::1-Stk',
    }),
    offer({
      title: 'Greisinger Berner Wuerstel',
      categoryPrimary: 'Lebensmittel',
      categorySecondary: 'Fleisch, Wurst & Fisch',
      comparisonGroup: 'greisinger-berner-wuerstel::0.4-kg',
    }),
    offer({
      title: 'Wassereis Swip',
      categoryPrimary: 'Lebensmittel',
      categorySecondary: 'Eis',
      comparisonGroup: 'wassereis-swip::0.66-l',
    }),
  ];

  assert.deepEqual(applyQueryMatch(offers, 'reis'), []);
});
...
test('keeps milchreis and reiswaffeln as weaker plausible rice-related hits', () => {
  ...
  assert.ok(sortedTitles.indexOf('Milchreis pur') > 0);
  assert.ok(sortedTitles.indexOf('Reiswaffeln Natur') > 0);
});
...
test('final response dedupe keeps distinct coffee flyer offers visible', () => {
  const coffeeOffers = [
    offer({
      title: 'REGIO Gold 500 g',
      titleNormalized: 'regio gold 500 g',
      retailerKey: 'spar',
    ...
    offer({
      title: 'Tassimo Kaffeekapseln',
      titleNormalized: 'tassimo kaffeekapseln',
      retailerKey: 'spar',
      sourceType: 'aktionsfinder-json',
      priceCurrent: { amount: 4.49 },
      quantityText: '16 Stk',
    }),
    offer({
      title: 'Nescafe Classic Loeskaffee 200 g',
      titleNormalized: 'nescafe classic loeskaffee 200 g',
      retailerKey: 'spar',
      sourceType: 'aktionsfinder-json',
      priceCurrent: { amount: 6.99 },
      quantityText: '200 g',
    }),
    offer({
      title: 'Cafe Royal Kapseln',
      titleNormalized: 'cafe royal kapseln',
      retailerKey: 'spar',
      sourceType: 'aktionsfinder-json',
      priceCurrent: { amount: 3.49 },
      quantityText: '10 Stk',
    }),
    offer({
      title: 'Meinl Praesident 500 g',
      titleNormalized: 'meinl praesident 500 g',
      retailerKey: 'spar',
      sourceType: 'aktionsfinder-json',
    ...
    offer({
      ...
      titleNormalized: 'dallmayr prodomo 500 g',
      retailerKey: 'spar',
      sourceType: 'aktionsfinder-json',
      priceCurrent: { amount: 7.49 },
      quantityText: '500 g',
    }),
  ];

  assert.equal(dedupeFinalResponseOffers(coffeeOffers, 'kaffee').length, coffeeOffers.length);
});
