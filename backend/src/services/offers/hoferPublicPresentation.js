...
const HOFER_TECHNICAL_TITLE_PATTERN = /^(?:max\.?\s|gr\.?\s|ca\.?\s|kapazit(?:aet|ät)|kochkapazit(?:aet|ät)|umfang\s*:|verschi(?:e|ä)dene\s+(?:farben|groessen|größen|modelle|designs)|(?:und\s+)?modelle\b|beim\b.*\bstatt|werbetermin\b|t\s*ef\b|tiefpreis\s*$|xxl[- ]?packung\s+sortiments|xxl\s+kefir\s+sortiments|sortiments[- ]?artikel|hofer\s*\|\s*\d+hofer\s*\|\s*\d+|alle\s+angebote\s+sind\s+online\s+buchbar)/i;
