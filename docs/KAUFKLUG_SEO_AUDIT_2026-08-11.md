...
## Trust-, Betreiber- und Methodiksignal 2026-08-28

Der technische und inhaltliche Audit zeigte keinen Anlass für ein erfundenes `Organization`-Entity: Kaufklug ist laut Impressum ein privates Projekt von Mag. Andreas Franz MA, nicht eine belegte eigene Gesellschaft. Das vorhandene WebSite-/WebApplication-/Person-Markup und die funktionierende `SearchAction` bleiben bestehen. Die belastbaren Trust-Aussagen waren jedoch auf Impressum, Nutzungshinweise, FAQ und Beta-Hinweis verteilt; eine zentrale indexierbare Produkt-/Methodikseite fehlte. `/so-funktioniert-kaufklug/` war vor dem Eingriff eine 200-Catchall-Fehlerseite mit `noindex,nofollow` ohne Canonical.
...
### Deployment und Live-Smoke

Die Commits `9ebe24e2` und `83714713` wurden auf `origin/main` gepusht und vom regulären Static-Deploy übernommen. Neue Live-Evidence: `Last-Modified: Wed, 02 Sep 2026 10:46:43 GMT`, Bundle `index-Dlyccc3_.js`; der Backend-Prozess blieb beim Static-only-Deploy erwartungsgemäß unverändert und meldet Health 200 sowie Mongo verbunden.

Die Live-Matrix für Bier, Lidl, BILLA, PENNY, Nudeln, Duschgel, Chips, Schokolade und Kaffee ist vollständig grün: jeweils HTTP 200, route-spezifischer Title und H1, vollständiger sichtbarer Einstieg im initialen HTML, Self-Canonical mit Slash, canonical Breadcrumb-Item und keine crawlbaren Non-Slash-Angebotslinks. Robots sind auf acht Seiten `index,follow`; Chips ist bei 8 Public Offers korrekt `noindex,follow`. Die Live-Sitemap enthält 25 ausschließlich canonical URLs, darunter Bier, Lidl, BILLA, PENNY, Nudeln, Duschgel, Schokolade und Kaffee; Chips bleibt fail-closed draußen. Homepage und Top Deals verlinken Bier, Lidl, BILLA und PENNY crawlbar.

Der repository-eigene vollständige 390-px-Live-Browser-Smoke meldet `passed` für Homepage, Methodikseite, Reduced Motion, Kaffee-/Waschmittel-/Biersuche, Browse, Top Deals, Einkaufsliste und Listenroute. Seine vorgeschalteten Node-`fetch`-Preflights wurden im lokalen Runner wegen eines HTTPS-Transportfehlers übersprungen; die eigentlichen Browserassertions sowie die unabhängige direkte HTTP-/SEO-Matrix waren grün. Keine Rankingwirkung wird aus dem erfolgreichen Deploy abgeleitet.

## Messvertrag für 2026-09-09

Dieser Eintrag dokumentiert ausschließlich die nächste Auswertung; er ändert weder SEO-Code noch Search-Console-Konfiguration. Verwendet wird dieselbe Kaufklug-Property mit Search Type `web`, Land `AUT` und finalen Daten. Maßgeblich ist das jüngste gemeinsame, vollständig verfügbare Enddatum. Verglichen werden dann exakt 7 vollständige Tage gegen die unmittelbar vorherigen 7 vollständigen Tage sowie 28 vollständige Tage gegen die unmittelbar vorherigen 28 vollständigen Tage.

Für jedes Fenster werden Klicks, Impressionen, CTR und durchschnittliche Position ausgewertet. Fokusgruppen sind Bier, Lidl, BILLA, PENNY, Kaffee, Nudeln, Duschgel und Schokolade; Chips kommt nur hinzu, wenn die Seite wieder belastbar indexiert ist. HOFER wird als bestehendes Händlerexperiment kontrolliert. SPAR, EUROSPAR und INTERSPAR werden als gemeinsame neue Beobachtungsgruppe aufgenommen: Der reguläre Full Crawl `6a99a3f09b4fb1f3ffa7b272` hat ihren Current-Flyer-Betrieb am 2026-09-03 stabil bestätigt. Aus dem Crawl-Fix allein wird keine organische Wirkung abgeleitet.

Je Fokusgruppe werden Query- und Page-Zeilen betrachtet. Slash- und Non-Slash-URLs werden zunächst getrennt ausgewiesen und anschließend zusätzlich kombiniert: Klicks und Impressionen werden summiert, die kombinierte CTR wird aus den Summen neu berechnet und die Position impressionsgewichtet aggregiert. Dadurch bleibt die bekannte URL-Aufteilung sichtbar, ohne dieselbe Landingpage in der Gesamtbewertung doppelt zu interpretieren.

Entscheidungen folgen erst aus der vollständigen Vergleichsmatrix und ausreichender Evidence. Es gibt in diesem Schritt keine automatische Search-Console-Abfrage, keine Einreichung, keine Reindex-Aktion, keine neue Landingpage und keine vorweggenommene Title-, Description-, Canonical-, Redirect- oder interne-Link-Änderung.
